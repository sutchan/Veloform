...
Veloform 采用扁平化组件架构，所有组件均为 standalone components，避免模块嵌套带来的复杂性。状态管理通过 **ConfigStore** 和 **ConfigService** 集中处理。
...
|------|------|------|--------|---------|
| `NavbarComponent` | Smart | 导航、认证、主题/语言切换 | — | `openLibrary` |
| `SidebarComponent` | Presentational | 车型选择 | `activeType` | `typeSelected` |
| `PreviewComponent` | Presentational | Three.js 3D 预览 + 统计页脚 | `name`, `type`, `weight`, `cost` | — |
| `BuildListComponent` | Presentational | 组件列表、编辑操作 | `components`, `isSaving` | `sync`, `deploy`, `edit` |
...
| `NotificationDisplayComponent` | Smart | 通知显示（服务驱动） | — | — |
